...
const TravelDetail = ({ loaderData }: Route.ComponentProps) => {
  ...
  const imageUrls = data?.trip?.imageUrls || [];
  const tripData = parseTripData(data?.trip?.tripDetail);
  const paymentLink = data?.trip?.payment_link;
  ...
  const allTrips = (data?.allTrips as Trip[]) || [];
        ...
        {paymentLink && (
          <section className="flex justify-center mt-8">
            <a href={paymentLink} target="_blank" rel="noopener noreferrer">
              <ButtonComponent
                type="button"
                className="button-class !h-12 !w-full md:!w-[300px]"
              >
                <span className="p-16-semibold text-white">Book This Trip</span>
              </ButtonComponent>
            </a>
          </section>
        )}
